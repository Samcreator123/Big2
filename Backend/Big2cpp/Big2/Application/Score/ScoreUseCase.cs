...
using Big2.Domain.Services.Scoring.ValueObjects;
...
namespace Big2.Application.Score
{
    public class ScoreUseCase(ILog log, IScoreRepository repository)
    {
        public ScoreResponse Execute(ScoreRequest request)
        {
            try
            {
                log.LogTrace($"玩家 {request.FinishedPlayerID} 於遊戲 {request.GameID} 出牌結束，開始計分", request);
                ...
                log.LogTrace($"玩家 {request.FinishedPlayerID} 於遊戲 {request.GameID} 計分結束", request);
